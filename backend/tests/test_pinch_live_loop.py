...
def test_the_endpoints_answer_in_both_modes(client, mode, monkeypatch):
    ...
    if mode == "live":
        ...
        #
        # Cleared explicitly rather than relying on the machine not having any:
        # on a developer box with real credentials in .env.local the poll would
        # succeed and this would fail, so the suite went red exactly on the
        # machines that are set up to run the live demo.
        monkeypatch.setattr(settings, "PINCH_APPLICATION_ID", "")
        monkeypatch.setattr(settings, "PINCH_SECRET_KEY", "")
